...
public class RequiredIHEParameterValidator implements ConstraintValidator<RequiredIHEParameter, IHESearchCriteria> {
	...
	@Override
	public boolean isValid(IHESearchCriteria iheSearchCriteria, ConstraintValidatorContext constraintValidatorContext) {
		// case request Patient
		if (Objects.equals(IHERequestType.PATIENT, iheSearchCriteria.getRequestType())
				&& iheSearchCriteria.getPatientID() != null && !iheSearchCriteria.getPatientID().isBlank()) {
			return true;
		}
		// case request Study
		else {
			return Objects.equals(IHERequestType.STUDY, iheSearchCriteria.getRequestType())
					&& (!iheSearchCriteria.getStudyUID().isEmpty() && iheSearchCriteria.getAccessionNumber().isEmpty()
